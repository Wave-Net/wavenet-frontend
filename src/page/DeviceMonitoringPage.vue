...
watch(
  ...
  (newStatMessage) => {
    ...
    if (newStatMessage && newStatMessage.data) {
      ...
      const hasChanged =
        JSON.stringify(newStatMessage.data) !== JSON.stringify(previousData);

      // 데이터가 변경되었다면 is_transmitting 값을 업데이트
      if (hasChanged) {
        // 패킷을 보냈거나 받은 경우를 확인합니다.
        const hasPacketSentOrReceived = newStatMessage.data.some(
          (item) => item.stats.send_pkt > 0 || item.stats.recv_pkt > 0
        );
        data.value.children = generateChildNodes(iot.value.length);
...
.custom-datatable >>> .p-column-header-content {
  ...
  /* 세로 중앙 정렬 */
  height: 100%;
  /* 필요한 경우, 높이를 100%로 설정하여 가운데 정렬 보장 */
}
...
.chart
  >>> .p-organizationchart-lines[data-pc-section="lines"][style*="visibility: hidden"] {
  display: none;
}
